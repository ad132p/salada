...
func BlogRoutes(router *gin.Engine, blogController *controller.BlogController) {
	...
	{
		postRoutes.GET("/", blogController.GetRecentPosts)
		postRoutes.GET("/all", blogController.GetAllPosts)
		postRoutes.POST("/", middleware.AuthenticateMiddleware, blogController.CreatePost)
		postRoutes.POST("/comment", blogController.CreateComment)
		postRoutes.GET("/:slug", blogController.GetPostAndCommentsBySlug)
